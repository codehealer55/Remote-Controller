...
def receive_bitmap_data(client_socket):
    while True:
        try:
            ...
            bf_type, bf_size, bf_reserved1, bf_reserved2, bf_off_bits = struct.unpack(
                "<HIHHI", bfh
            )
            ...
            (
                ...
                bi_clr_used,
                bi_clr_important,
            ) = struct.unpack("<IIIHHIIIIII", bih)
            print(
                f"bi_width: {bi_width}, bi_height: {bi_height}, bi_bit_count: {bi_bit_count}"
            )
            ...
            print(
                f"Sample pixel (0,0): {image_np[0,0]}"
            )  # Print sample pixel value for debugging

            yield (
                b"--frame\r\n"
                b"Content-Type: image/jpeg\r\n\r\n"
                + cv2.imencode(".jpg", image_np)[1].tobytes()
                + b"\r\n"
            )
...
def connect_to_server():
    server_ip = (
        "127.0.0.1"  # Change to the IP address of your C++ server if it's remote
    )
    server_port = 23456
...
def video_feed():
    ...
    print("123123")
    return Response(
        receive_bitmap_data(client_socket),
